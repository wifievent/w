...
WIp WRtm::findGateway(std::string intfName, WIp ip) {
	for (WRtmEntry& entry: *this) {
		if (entry.intf() == nullptr) continue;
		if (entry.intf()->name() != intfName) continue;
		if (entry.gateway_ == WIp::nullIp()) continue;
...
#ifdef WOS_LINUX
bool WRtm::checkA(char* buf, WRtmEntry* entry) {
	char gateway[256];
	char intf[256];
	int metric;
	// default via 10.2.2.1 dev eth0 proto dhcp metric 100 (A)
...
bool WRtm::checkB(char* buf, WRtmEntry* entry) {
	char cidr[256];
	char intf[256];
	char myip[256];
	int metric;
...
bool WRtm::checkC(char* buf, WRtmEntry* entry) {
	char gateway[256];
	char intf[256];
	// default via 10.2.2.1 dev wlan0  table 1021  proto static (C)
...
bool WRtm::checkD(char* buf, WRtmEntry* entry) {
	char cidr[256];
	char intf[256];
	char ip[256];
	// 10.2.2.0/24 dev wlan0  proto kernel  scope link  src 10.2.2.189 (D)
...
bool WRtm::decodeCidr(std::string cidr, uint32_t* dst, uint32_t* mask) {
...
uint32_t WRtm::numberToMask(int number) {
	uint32_t res = 0;
