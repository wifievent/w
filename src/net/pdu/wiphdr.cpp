...
//
uint16_t WIpHdr::calcChecksum(WIpHdr* ipHdr) {
	uint32_t res = 0;
	uint16_t *p;

	// Add ipHdr buffer as array of uint16_t
	p = reinterpret_cast<uint16_t*>(ipHdr);
	for (int i = 0; i < int(sizeof(WIpHdr)) / 2; i++) {
		res += ntohs(*p);
		p++;
	}
...
uint16_t WIpHdr::recalcChecksum(uint16_t oldChecksum, uint16_t oldValue, uint16_t newValue) {
...
uint16_t WIpHdr::recalcChecksum(uint16_t oldChecksum, uint32_t oldValue, uint32_t newValue) {
	uint16_t oldValue16;
	uint16_t newValue16;
	uint16_t res;
